...
from app.views.views import findAttributeByNameAndTable, findTableByName, TreeView
...
urlpatterns = [
    ...
    path('queries/search/', QueryViewSet.as_view({'get': 'get'}), name='query_search'),
    path('queries/updateJoinOrder', QueryViewSet.as_view({'put': 'updateJoinOrder'}), name='updateJoinOrder'),
    path('tables/<str:table_name>/attributes/<str:attribute_name>/', findAttributeByNameAndTable,name='findAttributeByNameAndTable'),
    path('tables/<str:table_name>/', findTableByName, name='get_table'),
    path('tree-view/', TreeView.as_view(), name='tree_view'),
]
